...
#include <thread>    // Se importa la libreria para el manejo de hilos concurrentes
...
int dinamicPassword = (rand() % 900000 + 100000); // Variable global para almacenar la contraseña dinámica
const int directions[5][4] = {
    ...
    {4, 19, 7, 29},
    {5, 95, 11, 49}};
/*Matriz que almacena las direcciones de los establecimientos
 ID
 CALLE
 CARRERA
 # ESTABLECIMIENTO*/
...
void generateDinamicPassword(int *password)
{
    while (true)
    {
        this_thread::sleep_for(chrono::seconds(3)); // Detiene el hilo de la generacion de la clave dinamica por 3 segundos
...
void onLoginMenu(string *phoneNumber)
{
    ...
    do
    {
        ...
        println("7. Buscar Cajeros Disponibles"); 
        println("8. Salir");
...
void registerMenu()
{
    ...
    {
        println("El número de celular debe tener 10 digitos.");
        return;
    }
    ...
    if (clave.length() != 4)
    {
        println("La clave debe tener 4 caracteres.");
        return;
    }
